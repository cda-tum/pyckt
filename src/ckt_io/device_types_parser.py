...
:class:`~core.DeviceTypeRegister` with the pin and tech-type
information for each device category (Mosfet, Capacitor, …).

The file defines:
  - Which :class:`~core.PinType` values each device type uses.
  - Which pins are optional (Bulk) and their auto-connection rules.
  - Which :class:`~core.TechType` flavours exist per device type.
