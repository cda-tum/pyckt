...
Reads an ``.hspice`` file and produces a fully-wired :class:`~core.Circuit`
by combining three configuration sources:

  - :class:`~ckt_io.hspice_mapping.HSpiceMapping` — device-line interpretation
  - :class:`~ckt_io.supply_nets_parser.SupplyNetConfig` — supply/ground labels
  - :class:`~core.DeviceTypeRegister` — pin metadata (optional pins, etc.)
...
class HSpiceParser:
    """Parse an HSpice circuit netlist into a :class:`~core.Circuit`.
    ...
    def _apply_supply_classification(self, circuit: Circuit) -> None:
        """Tag nets with :class:`~core.Supply` based on :class:`SupplyNetConfig`.
