...
from pyckt.io.acst_xml import make_root, slash, write_tree
...
class AcstSizingXMLWriter:
	...
	* leading-slash component/transistor names.
	...
	Tier-2 sections that acst emits but pyckt does not yet compute — net
	``<Voltages>``, ``<Capacitors>``, ``CMRR``/``PSRR``, common-mode input
	ranges — are omitted.
	"""
