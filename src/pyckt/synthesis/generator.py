...
and packages them into a :class:`~pyckt.synthesis.library.TopologyLibrary`.
...
class TopologyLibraryGenerator:
    ...
    * A **two-stage** op-amp for every ``InvertingStage`` variant (12 variants).
    ...
    Each op-amp is described by a :class:`~pyckt.synthesis.library.TopologySpec`
    and converted to a flat :class:`~pyckt.core.Circuit` via
    :class:`~pyckt.synthesis.converter.TopologyConverter`.
