...
1. **initialize()** — parse the circuit-specification XML to obtain a
   :class:`~pyckt.io.circuit_info_parser.Specifications` object and
   (optionally) load a pre-built topology library from disk or generate
   a fresh one via :class:`~pyckt.synthesis.generator.TopologyLibraryGenerator`.
2. **compute()** — instantiate a :class:`~pyckt.synthesis.engine.SynthesisEngine`
   and run :meth:`~pyckt.synthesis.engine.SynthesisEngine.synthesize`.
...
class SynthesisAnalysis(AbstractAnalysis):
    ...
    def initialize(self) -> None:
        ...
        1. Parse ``args.xml_spec_file`` for a
           :class:`~pyckt.io.circuit_info_parser.Specifications` object.
           If ``args.xml_tech_file`` is also provided, load
           :class:`~pyckt.io.technology_parser.TechnologyParams` as well.
        2. If ``args.library_dir`` is set and the directory exists, load the
           library via :meth:`~pyckt.synthesis.library.TopologyLibrary.from_directory`.
           Otherwise generate a fresh library with
           :class:`~pyckt.synthesis.generator.TopologyLibraryGenerator` and
        ...
        from pyckt.io.circuit_info_parser import parse_specifications
